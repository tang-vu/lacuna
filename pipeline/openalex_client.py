...
class OpenAlexClient:
    ...
    def __init__(self, cache_dir: Path = CACHE_DIR, use_cache: bool = True):
        ...
        self.session.headers["User-Agent"] = f"lacuna/0.1 (mailto:{MAILTO})"
    ...
    def build_url(self, path: str, params: dict[str, Any] | None = None) -> str:
        """Build a full API URL. The result is what gets stored as provenance, so it must be
        the literal string a reader can paste into a browser."""
    ...
    def _cache_path(self, url: str) -> Path:
        # Strip credentials before hashing so a cache stays valid whether or not a key is set.
        stable = url.replace(f"&api_key={API_KEY}", "") if API_KEY else url
    ...
    def get(self, path: str, params: dict[str, Any] | None = None, max_retries: int = 4) -> dict:
        ...
        for attempt in range(max_retries):
            ...
            if response.status_code == 200:
                ...
                payload["_lacuna_source_url"] = url
            ...
            raise RuntimeError(f"OpenAlex {response.status_code} for {url}: {response.text[:300]}")

        raise RuntimeError(f"OpenAlex unreachable after {max_retries} attempts: {url}")
